...
Table FollowsClause::HandleIntegerWildcard() {
  bool is_false_clause = pkb->GetFollowsStore()->GetBaseFollowingOf(first_arg.value)=="0";
  Table table;
  if (is_false_clause) {
    table.ToggleFalseClause();
  }
...
Table FollowsClause::HandleIntegerInteger() {
  bool is_false_clause = pkb->GetFollowsStore()->GetBaseFollowingOf(first_arg.value)!=second_arg.value;
  Table table;
  if (is_false_clause) {
    table.ToggleFalseClause();
  }
