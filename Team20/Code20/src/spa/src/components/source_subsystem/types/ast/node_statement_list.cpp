...
std::string StatementListNode::Process(Populator populator, std::vector<std::string> *visited, bool is_uses, std::shared_ptr<source::CfgProcedureNode> cfg_proc_node, std::shared_ptr<source::CfgGroupNode> cfg_node, std::string proc_name) {
  ...
  for (int i = 0; i < stmts.size(); ++i) {
    ...
    stmt->Process(populator, visited, false, cfg_proc_node, cfg_node, proc_name);
  }
  return "";
}
