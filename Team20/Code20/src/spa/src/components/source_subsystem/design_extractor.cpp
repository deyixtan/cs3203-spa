  ...
  for (int i = 0; i < stmts.size(); ++i) {
    ...
    switch(stmt_type) {
      ...
      case READ: {
        ...
        PopulateModifiesProc(proc->GetIdentifier(), var_name);
        break;
      }
      case PRINT: {
        PopulateStmt(stmt_num);
        ...
        PopulateUsesProc(proc->GetIdentifier(), var_name);
        break;
      }
      case ASSIGN: {
        PopulateStmt(stmt_num);
        ...
        std::string rhs_expr = ExprNodeHandler(visited, proc, stmt_num, expr, 0, "");
        //rhs_expr.erase(0, 1);
      ...
      case WHILE: {
        ...
        CondExprNodeHandler(visited, proc, stmt_num, while_stmt_cond);
      ...
      case IF: {
        ...
        CondExprNodeHandler(visited, proc, stmt_num, if_stmt_cond);
...
void DesignExtractor::CondExprNodeHandler(std::vector<std::string> visited, std::shared_ptr<ProcedureNode> proc, std::string stmt, std::shared_ptr<ConditionalExpressionNode> if_stmt_cond) {
  ...
  switch(cond) {
    case ConditionalType::BOOLEAN: {
      ...
      CondExprNodeHandler(visited, proc, stmt, left);
      CondExprNodeHandler(visited, proc, stmt, right);
      break;
    }
    case ConditionalType::NOT: {
      ...
      CondExprNodeHandler(visited, proc, stmt, expr);
      break;
    }
    case ConditionalType::RELATIONAL: {
      ...
      ExprNodeHandler(visited, proc, stmt, left);
      ExprNodeHandler(visited, proc, stmt, right);
      break;
    }
  }
}

void DesignExtractor::ExprNodeHandler(std::vector<std::string> visited, std::shared_ptr<ProcedureNode> proc, std::string stmt, std::shared_ptr<ExpressionNode> expr) {
  ...
  switch (expr_type) {
    ...
    case ExpressionType::COMBINATION: {
      ...
      ExprNodeHandler(visited, proc, stmt, lhs);
      ExprNodeHandler(visited, proc, stmt, rhs);
      break;
    }
...
std::string DesignExtractor::ExprNodeHandler(std::vector<std::string> visited, std::shared_ptr<ProcedureNode> proc, std::string stmt_num, std::shared_ptr<ExpressionNode> expr, int direction, std::string pattern) {
  ExpressionType expr_type = expr->GetExpressionType();
  ...
  switch (expr_type) {
    ...
    case ExpressionType::COMBINATION: {
      ...
      ExprNodeHandler(visited, proc, stmt_num, lhs, 1, pattern);
      ExprNodeHandler(visited, proc, stmt_num, rhs, 2, pattern);
...
void DesignExtractor::PopulateUsesProc(std::string proc, std::string var) {
  this->pkb->GetUsageStore()->AddProcVar(proc, var);
}
...
void DesignExtractor::PopulateModifiesProc(std::string proc, std::string var) {
  this->pkb->GetModifyStore()->AddProcVar(proc, var);
}
