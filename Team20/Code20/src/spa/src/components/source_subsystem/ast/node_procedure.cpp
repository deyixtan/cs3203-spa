...
std::string ProcedureNode::Process(Populator populator, std::vector<std::string> *visited, std::shared_ptr<source::CfgProcedureNode> cfg_proc_node, std::shared_ptr<source::CfgGroupNode> cfg_node) {
  populator.PopulateProc(m_identifier);
  return m_stmt_list->Process(populator, visited, cfg_proc_node, cfg_node);
}
